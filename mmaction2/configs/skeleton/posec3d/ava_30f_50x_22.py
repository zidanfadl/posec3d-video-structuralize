...
ann_file_train = 'data/ava_tubelet/ava_kp_train_2.2.pkl'
ann_file_val = 'data/ava_tubelet/ava_kp_val_2.2.pkl'
...
test_pipeline = [
    ...
    dict(type='Resize', scale=(-1, 64)),
    dict(type='CenterCrop', crop_size=64),
    dict(type='GeneratePoseTarget', sigma=0.6, use_score=True, with_kp=True, with_limb=False, double=False, left=left, right=right),
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs'])
]
...
workflow = [('train', 1)]
evaluation = dict(
    interval=1, metrics=['mean_average_precision'], key_indicator='mAP')
log_config = dict(
    interval=20,
    hooks=[
...
log_level = 'INFO'
work_dir = './work_dirs/ava_tubelet/ava_30f_50x_22'
load_from = None
resume_from = None
